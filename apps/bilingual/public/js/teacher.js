(() => {
  ...
  // Which language the downloadable report is generated in.
  let reportLang = 'en';
  let clockTimer = null;
  let endsAt = null;
  ...
  function unlock() {
    ...
    socket.emit('teacher:auth', { passcode }, (res) => {
      ...
      catalog = res.models || [];
      $('translation-note').textContent = res.translation
        ? 'Messages between participants who pick different languages are translated automatically.'
        : 'No API key — cross-language messages will be delivered untranslated.';
  ...
  for (const button of document.querySelectorAll('[data-report-lang]')) {
    button.addEventListener('click', () => {
      reportLang = button.dataset.reportLang;
      for (const other of document.querySelectorAll('[data-report-lang]')) {
        other.classList.toggle('on', other === button);
      }
      // Anything already downloaded was in the other language, so offer it again.
      downloaded = false;
  ...
  $('zip-btn').addEventListener('click', () => {
    ...
    socket.emit('teacher:report', { lang: reportLang }, (res) => {
      if (!res?.ok) {
  ...
  function renderLobby() {
    ...
    for (const student of students) {
      ...
      chip.innerHTML = `<div class="who">${student.student}</div>` +
        `<div class="sub">${student.code} · ${student.lang === 'zh' ? '中文' : 'EN'}</div>` +
        '<span class="dot"></span>';
    ...
    for (const student of (latest.students || []).filter((x) => x.inRound)) {
      ...
      cell.innerHTML = `
        ...
        <div class="sub">${student.partnerType === 'ai'
          ...
          : 'peer'} · ${student.lang === 'zh' ? '中文' : 'EN'}</div>`;
