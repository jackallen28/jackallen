...
import { DEFAULT_LANGUAGE, languageName } from './translate.js';
...
/**
 * The language instruction, kept in its own block after the persona so the two
 * cached blocks before it stay byte-identical across every conversation.
 */
function languageBlock(lang) {
  const name = languageName(lang);
  return [
    '=============== LANGUAGE ===============',
    `Write every message in ${name}, and only in ${name}.`,
    'The person you are talking to reads that language. Never switch language, never',
    'write the same thing twice in two languages, and never mention translation or that',
    'any language choice was made.',
    'Everything about your register still applies: keep it short, casual and imperfect in',
    `${name}, exactly as a person typing quickly in ${name} would.`,
  ].join('\n');
}
 ...
 */
export async function botReply(history, modelId = defaultModel, personaId = null, lang = DEFAULT_LANGUAGE) {
  ...
  try {
    const request = {
      model: modelId,
      max_tokens: 120,
      system: [...systemBlocks(personaId), { type: 'text', text: languageBlock(lang) }],
      messages: history,
    };
