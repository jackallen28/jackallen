...
import { LANGUAGES, DEFAULT_LANGUAGE, isLanguage, isTranslationConfigured } from './translate.js';
import { REPORT_LANGUAGES } from './i18n.js';
...
console.log(
  isTranslationConfigured()
    ? '[server] Translation between English and 中文 is available.'
    : '[server] No API key — cross-language messages will be delivered untranslated.'
);
...
io.on('connection', (socket) => {
  ...
  socket.on('student:join', (payload, ack) => {
    const result = session.join(payload?.code, socket.id, payload?.lang);
